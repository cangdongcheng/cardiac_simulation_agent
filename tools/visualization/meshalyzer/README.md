...
| File | Status | Topic |
| --- | --- | --- |
| `haswell-fix.md` | validated | Binary patch for shader compilation + vertex picking on Intel Haswell / WSL |
...
- `troubleshooting.md` — for issues other than the Haswell shader bug.

When the user runs into the Haswell shader compilation or vertex-picking issue, open `haswell-fix.md` directly — that note has the symptoms in its `use_when:` frontmatter.
